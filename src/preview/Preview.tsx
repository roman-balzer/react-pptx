...
const normalizeBorderToCSS = (style: InternalTableStyle) =>
    `${style.borderWidth ?? 0}px solid ${
        style.borderColor ? normalizedColorToCSS(style.borderColor) : undefined ?? 'transparent'
    }`
...
const SlideObjectShape = ({ shape }: { shape: InternalShape }) => {
    const baseStyle = {
        width: '100%',
        height: '100%',
        backgroundColor: shape.style.backgroundColor ? normalizedColorToCSS(shape.style.backgroundColor) : undefined,
    }
    ...
    if (shape.type === 'rect') {
        ...
        return (
            ...
            >
                ...
                >
                    <span style={{ backgroundColor: 'white', padding: 2 }}>no preview for "{shape.type}"</span>
                </div>
            </div>
        )
    }
...
const getTextStyleForPart = (
    ...
    const pointsToPx = (points: number) => ((points * POINTS_TO_INCHES) / dimensions[0]) * slideWidth

    let margin: string | undefined = undefined
    if (style.margin !== undefined) {
        if (Array.isArray(style.margin)) {
            margin = style.margin.map((marginItem) => `${pointsToPx(marginItem)}px`).join(', ')
        } else {
    ...
    const textDecoration = textDecorationParts.length ? textDecorationParts.join(' ') : undefined
    ...
    return {
        ...
        padding: margin,
        lineHeight: style.lineSpacing ? `${pointsToPx(style.lineSpacing)}px` : undefined,
        letterSpacing: style.charSpacing ? `${pointsToPx(style.charSpacing)}px` : undefined,
        textDecoration,
        marginTop: style.paraSpaceBefore ? `${pointsToPx(style.paraSpaceBefore)}px` : undefined,
        marginBottom: style.paraSpaceAfter ? `${pointsToPx(style.paraSpaceAfter)}px` : undefined,
...
const TextPreview = ({
    ...
    // <ul> or <ol>
    const listsOfParts: ListParts[] = parts.reduce((collectedSoFar: ListParts[], part) => {
        if (!part.bullet) {
        ...
        <>
            {listsOfParts.reduce((elements, { listType, parts }, index) => {
                if (!listType) {
                    ...
                    const listElement =
                        listType === 'number' ? <ol key={index}>{listParts}</ol> : <ul key={index}>{listParts}</ul>
...
const constrainObjectFit = (sizing: InternalImage['style']['sizing']): 'contain' | 'cover' | undefined => {
    ...
    return (
        ...
        >
            {object.kind === 'text' ? (
                ...
                <img
                    src={
                        object.src.kind === 'data' ? `data:${object.src[object.src.kind]}` : object.src[object.src.kind]
                    }
                    style={{
                        width: '100%',
                        height: '100%',
                                ...
                                {row.map((cell: InternalTableCell, j: number) => {
                                    return (
                                        <td
                                            key={`${i}-${j}`}
                                            style={{
                                                ...getTextStyleForPart(cell.style, dimensions, slideWidth),
                                                border: normalizeBorderToCSS(object.style),
    ...
    return (
        <div
            ...
            style={{
                ...
                backgroundColor: backgroundColor ? normalizedColorToCSS(backgroundColor) : 'white',
                backgroundImage:
...
const Preview = (props: {
    ...
    try {
        const normalized = normalizePresentation(presentationNode)
